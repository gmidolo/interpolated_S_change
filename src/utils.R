...
format_ReSurveyEurope <- \(training_strategy = c('start', 'end', 'random', 'full', 'start_or_end'),
                           path_resurvey_clean = './data/input/ReSurveyEU_clean.csv.gz'
){
  
  if (!(training_strategy %in% c('start', 'end', 'random', 'full', 'start_or_end'))) {
    stop('Wrong training_strategy: it must be either "start", "end", "random", "start_or_end", or "full"!')
  }
  ...
  if (training_strategy == 'start_or_end') {
    traintest_data_start <- resurvey_clean %>%
      group_by(resurv_id) %>%
      arrange(year) %>%
      slice_head(n = 1) %>%
      ungroup()
    traintest_data_end <- resurvey_clean %>%
      group_by(resurv_id) %>%
      arrange(year) %>%
      slice_tail(n = 1) %>%
      ungroup()
    traintest_data <- bind_rows(traintest_data_start, traintest_data_end) %>%
      group_by(resurv_id) %>%
      sample_n(1) %>%
      ungroup()
    external_data <- resurvey_clean %>% anti_join(traintest_data,'plot_id')
...
format_lnRRchange_preds <- \(data, method = c('end.vs.start', 'random.within'), observed_value_name = 'S', predicted_value_name = '.pred'){
  ...
  if(method == 'random.within'){
    obs_vs_pred <- data %>%
      # if multiple years are available in a given midpoint, take just one randomly
      group_by(resurv_id, year) %>%
      slice_sample(n = 1) %>%
      ungroup() %>%
      # now sample two plots per resurvey, independently from their positioning along the time serie
      group_by(resurv_id) %>%
      slice_sample(n = 2) %>%
...
run_validation_RF <- \(datatype = c('EVA','ReSurv','EVA_ReSurv'), 
                       ...
                       trees_rf = 1000, mtry_rf = 4, min_n_rf = 2){
  ...
  if(datatype == 'EVA_ReSurv') {
    dat[['traintest_data']] <- bind_rows(dat[['traintest_data']], read_csv('./data/input/EVA.csv.gz', show_col_types = F))
  }
  ...
  ### 1 Internal evaluation (train vs test) ###
  # Define model
  spec <- rand_forest(
    trees = trees_rf, 
    min_n = min_n_rf,
