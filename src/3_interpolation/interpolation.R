...
# if FALSE, use the original plot size; if TRUE, set habitat median values
...
set.seed(123)
dat <- bind_rows(
  read_csv('./data/input/EVA.csv.gz', show_col_types = F),
  # Load EVA data
...
# Predict S for each plot in each of these years
...
for (i in pred_years) {
  ...
  pred_dat[paste0('S_pred_', i)] <- predict(object = m_wf, new_data = pdi) %>%  pull(.pred)
}
...
for (i in period) {
  ...
  res_lm_periods[[i]] <- pred_dat %>%
    ...
    setNames(c(
      'plot_id',
      paste0('lm.slope_estimate', i),
      paste0('lm.slope_std.error', i)
    ))
}
