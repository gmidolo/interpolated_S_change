...
dat <- './data/preds/preds_stdpltsz.rf.csv' %>%
 ...
 # restrict the analyses to plot within the studied period
...
# N.B.: Standard deviation, rather than the mean, can be calculated.
# To do this, use the `sd_at_least_five_plots` function, and pass it in place of `mean_at_least_five_plots`.
# sd_at_least_five_plots <- \(x, th = 5) {ifelse(length(x) >= th, sd(x), NA)}
...
# This can be changed to other metrics of change, i.e., lnRR, absolute change and the S~time slope estimate
...
for (h in c('Forest', 'Grassland', 'Scrub', 'Wetland')) {
  for (i in var2collect$var) {
    ...
    d_h_i <- d_h_i  %>%
      # # plot only over the plots originally sampled within each period?
      # # if these filters are applied, maps like those presented in the SI are plotted
